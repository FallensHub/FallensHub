...
function Orionlib_ui_ui:BuildMainFrame()
    ...
    if logoId:match("^rbxassetid://") or logoId:match("^rbxasset://") or logoId:match("^http") then
        -- Banner-sized logo: full header height, wide aspect ratio like in the reference image
        local bannerH = 56 * scale_factor
        local bannerW = 180 * scale_factor  -- wide banner width like the Nemesis logo reference
    ...
    -- Attach custom accent scrollbar to settings scroll
    attach_scrollbar(self, settings_scroll, _real_settings_panel, {
        TrackWidth = 5,
        ThumbWidth = 3,
        EdgeInset = 2,
        VerticalInset = 4,
    ...
    local function setSettingsPanelOpen(openState)
        ...
        if self.settings_open then
            ...
            -- Show scrollbar only if content taller than panel
            settings_scroll.ScrollBarImageTransparency = (contentH > maxH) and 0.2 or 1
