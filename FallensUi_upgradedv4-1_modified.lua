...
-- Custom Logo Image
local Orionlib_ui_img = writefile("Orionlib_ui_logo.gif", game:HttpGet(
    "https://raw.githubusercontent.com/FallensHub/FallensHub/refs/heads/main/1000008860%20(1).png"
))
local Orionlib_ui_logo = getcustomasset("Orionlib_ui_logo.gif")
...
function Orionlib_ui_ui:_RefreshAccentCore()
    ...
    if self._toggleStroke then
        self._toggleStroke.Color = accent
    end
    if self.toggle_frame then
        self.toggle_frame.BackgroundColor3 = self.config.BackgroundColor
    end
...
function Orionlib_ui_ui:BuildToggleButton()
    ...
    create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = self.toggle_frame})
    ...
    self._toggleStroke = toggle_stroke
    ...
    self.toggle_icon = create("ImageLabel", {
        ...
        Size = UDim2.new(0, btn_size * 0.75, 0, btn_size * 0.75),
        Parent = self.toggle_frame
    })
    ...
    toggle_btn.MouseButton1Click:Connect(function()
        self:Toggle()
        tween_to(self.toggle_icon, {Size = UDim2.new(0, btn_size * 0.55, 0, btn_size * 0.55)}, 0.1)
        task.delay(0.1, function()
            tween_to(self.toggle_icon, {Size = UDim2.new(0, btn_size * 0.75, 0, btn_size * 0.75)}, 0.18, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
        end)
    end)
...
function Orionlib_ui_ui:BuildMainFrame()
    ...
    local logoDisplayWidth = 0  -- track actual logo width for layout offset
    if logoId:match("^rbxassetid://") or logoId:match("^rbxasset://") or logoId:match("^http") then
        -- 1:1 square logo, sized to fill header height with padding
        local logoSize = 48 * scale_factor
        logoDisplayWidth = logoSize + 8 * scale_factor  -- size + gap
        ...
        self.hub_name_label = create("TextLabel", {
            ...
            AnchorPoint = Vector2.new(0, 0.5),
            Position = UDim2.new(0, headerLeftPadding, 0, 32 * scale_factor), TextSize = 22 * scale_factor,
            Size = UDim2.new(0, headerNameMaxWidth, 0, 28 * scale_factor),
    ...
    -- Hub name text beside logo (shown when logo is an image)
    if logoDisplayWidth > 0 then
        local nameX = headerLeftPadding + logoDisplayWidth
        local nameMaxW = math.max(120 * scale_factor, headerNameMaxWidth - logoDisplayWidth)
        local hubNameText = create("TextLabel", {
            FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.Bold),
            TextColor3 = Color3.new(1, 1, 1),
            Text = self.config.Name,
            BackgroundTransparency = 1,
            AnchorPoint = Vector2.new(0, 0.5),
            Position = UDim2.new(0, nameX, 0, 32 * scale_factor),
            TextSize = 22 * scale_factor,
            Size = UDim2.new(0, nameMaxW, 0, 28 * scale_factor),
            TextTruncate = Enum.TextTruncate.AtEnd,
            TextXAlignment = Enum.TextXAlignment.Left,
            Parent = self.main_frame
        })
        table.insert(self._gradientLabels, hubNameText)
    end
    
    -- user_name_label and avatar_image removed
    
    -- Separator spans full sidebar width
    local sidebarEndX = 187 * scale_factor
    ...
    -- Close/minimize button removed intentionally
    ...
    -- Sidebar scrollbar visual removed intentionally
    ...
    local settingsPanelWidth = 240 * scale_factor
    local settingsPanelHeight = 520 * scale_factor
    self.settings_open = false

    self.settings_btn_frame = create("Frame", {
        BackgroundColor3 = Color3.fromRGB(20, 20, 20),
        Position = UDim2.new(0, 10, 1, -31 * scale_factor),
        Size = UDim2.new(0, 145 * scale_factor, 0, 22 * scale_factor),
        BorderSizePixel = 0,
        Parent = self.main_frame
    })
    ...
    -- Inner scroll frame so content doesn't get clipped when panel is shorter than content
    local settings_scroll = create("ScrollingFrame", {
        BackgroundTransparency = 1,
        Position = UDim2.new(0, 0, 0, 0),
        Size = UDim2.new(1, 0, 1, 0),
    ...
    self:SetSmoothScroll(settings_scroll, 34)
    -- Redirect all settings_panel children creation to scroll frame
    -- We alias settings_panel to scroll so existing create() calls parent to scroll
    local _real_settings_panel = self.settings_panel
    self.settings_panel = settings_scroll
    ...
    -- overlayToggleRef removed (Overlay FX)
    ...
    -- hideNameToggleRef removed (user name label removed)
    ...
    self.settings_toggle_refs = {blurToggleRef, bgFxToggleRef, gradientToggleRef, espPreviewToggleRef}
    ...
    -- ── Change Keybind ──────────────────────────────────────────────────────
    rowY = rowY + 8 * scale_factor  -- extra gap between mobile toggle and key picker

    -- divider between toggle group and key picker
    create("Frame", {
        BackgroundColor3 = Color3.fromRGB(40, 40, 40),
        Position = UDim2.new(0, 10, 0, rowY - 4 * scale_factor),
        Size = UDim2.new(1, -20, 0, 1),
        BorderSizePixel = 0, Parent = self.settings_panel
    })
    rowY = rowY + 8 * scale_factor
    ...
    -- Config section moved to Menu Tab (use lib:AddConfigTab(tabObj) to populate)
    -- Overlay row removed
    ...
    -- Restore self.settings_panel to the real outer frame
    -- AutomaticCanvasSize = Y handles height automatically; set manual as fallback
    self.settings_panel = _real_settings_panel
    settings_scroll.CanvasSize = UDim2.new(0, 0, 0, rowY + 20 * scale_factor)

    -- No custom scrollbar visual on settings panel — native scroll only (invisible bar)
    settings_scroll.ScrollBarThickness = 0
    ...
    local function setSettingsPanelOpen(openState)
        ...
        if self.settings_open then
            ...
            local contentH = rowY + 20 * scale_factor
            -- Clamp panel height to available vertical space
            local maxH = contentH
            if self.main_frame and self.main_frame.Parent then
                local frameH = self.main_frame.AbsoluteSize.Y
                local panelBottom = frameH - 36 * scale_factor
                local topMargin = 8 * scale_factor
                maxH = math.max(80 * scale_factor, math.min(contentH, panelBottom - topMargin))
            end
            self.settings_panel.ClipsDescendants = true
            tween_to(self.settings_panel, {Size = UDim2.new(0, settingsPanelWidth, 0, maxH)}, 0.22)
            settings_scroll.Size = UDim2.new(1, 0, 1, 0)
...
--[[
    AddConfigTab(tabObj)
    Populates an existing tab with a full config management panel
    (Config Name input, Refresh/Load/Save/Delete/Import/Export/Autoload buttons).
    Usage:
        local cfg_tab = my_section:AddTab({Name = "Config", Icon = "save"})
        lib:AddConfigTab(cfg_tab)
--]]
function Orionlib_ui_ui:AddConfigTab(tabObj)
    if not tabObj or type(tabObj.AddGroup) ~= "function" then
        warn("[FallensUI] AddConfigTab: tabObj must be a valid tab returned from AddTab()")
        return
    end

    local lib = self

    -- Left group: Config file management
    local cfg_group = tabObj:AddGroup({Name = "Config Manager", Side = "Left", Icon = "hard-drive"})

    -- Config name input
    cfg_group:AddTextInput({
        Name = "Config Name",
        PlaceholderText = "Enter config name...",
        Default = "",
        Flag = "__cfg_name_input__",
    ...
    cfg_group:AddButton({
        ...
        Callback = function()
            ...
            local display = #names > 0 and table.concat(names, ", ") or "No configs found"
            lib:Notify({Title = "Configs", Description = display, Duration = 4})
        end
    })

    local function getConfigName()
        local inputObj = Options and Options["__cfg_name_input__"]
        if inputObj and type(inputObj.Get) == "function" then
            local v = inputObj:Get()
            if v and v ~= "" then return v end
        end
        return lib._autoConfigName
    end
    ...
    cfg_group:AddButton({
        Name = "Load Config",
        Callback = function()
            local ok, msg = lib:LoadConfig(getConfigName())
            lib:Notify({Title = "Config", Description = ok and "Config loaded!" or ("Load failed: " .. tostring(msg)), Duration = 3})
        end
    })
    ...
    cfg_group:AddButton({
        ...
        Callback = function()
            ...
            local ok, msg = lib:SaveConfig(name)
            lib:Notify({Title = "Config", Description = ok and ("Saved: " .. name) or ("Save failed: " .. tostring(msg)), Duration = 3})
        end
    })
    ...
    cfg_group:AddButton({
        ...
        Callback = function()
            local name = getConfigName()
            local path = get_config_folder() .. "/" .. get_config_filename(name)
            local deleted = false
            pcall(function()
            ...
            lib:Notify({Title = "Config", Description = deleted and ("Deleted: " .. name) or "File not found.", Duration = 3})
        end
    })

    -- Right group: Import / Export / Autoload
    local io_group = tabObj:AddGroup({Name = "Import / Export", Side = "Right", Icon = "arrow-left-right"})

    -- Import Code input
    io_group:AddTextInput({
        Name = "Import Code",
        PlaceholderText = "Paste config code here...",
        Default = "",
        Flag = "__cfg_import_code__",
        Callback = function() end
    })
    ...
    io_group:AddButton({
        Name = "Import Config",
        Callback = function()
            local inputObj = Options and Options["__cfg_import_code__"]
            local code = inputObj and type(inputObj.Get) == "function" and inputObj:Get() or ""
            if code == "" then
                lib:Notify({Title = "Import", Description = "Paste a config code first.", Duration = 2})
                return
            end
            local okDecode, data = pcall(function()
                return http_service:JSONDecode(code)
            end)
            if not okDecode or type(data) ~= "table" or type(data.controls) ~= "table" then
                lib:Notify({Title = "Import", Description = "Invalid config code.", Duration = 3})
                return
            end
            lib._isApplyingConfig = true
            for flag, rawValue in pairs(data.controls) do
                local control = lib._trackedControls[flag]
                if control then
                    local decodedValue = deserialize_value(rawValue)
                    pcall(control.set, decodedValue, false)
                end
            end
            lib._isApplyingConfig = false
            lib:Notify({Title = "Import", Description = "Config imported!", Duration = 2})
        end
    })
    ...
    io_group:AddButton({
        ...
        Callback = function()
            local payload = {version = 1, ui = lib.config.Name, controls = {}}
            for flag, control in pairs(lib._trackedControls) do
                local ok, value = pcall(control.get)
                if ok then payload.controls[flag] = serialize_value(value) end
            end
            local ok, encoded = pcall(function() return http_service:JSONEncode(payload) end)
            if ok and setclipboard then
                pcall(setclipboard, encoded)
                lib:Notify({Title = "Export", Description = "Config copied to clipboard!", Duration = 2})
            else
                lib:Notify({Title = "Export", Description = "Clipboard not available.", Duration = 2})
            end
        end
    })
    ...
    io_group:AddButton({
        ...
        Callback = function()
            local name = getConfigName()
            lib._autoConfigName = sanitize_config_name(name)
            lib._autoConfigEnabled = true
            lib._autoConfigLoadAttempted = false
            lib:Notify({Title = "Autoload", Description = "Set autoload: " .. name, Duration = 2})
        end
    })
    ...
    io_group:AddButton({
        Name = "Clear Autoload",
        Callback = function()
            local path = get_config_folder() .. "/" .. get_config_filename(lib._autoConfigName)
            pcall(function()
                if isfile and isfile(path) and delfile then delfile(path) end
            end)
            lib._autoConfigEnabled = false
            lib._autoConfigLoadAttempted = true
            lib:Notify({Title = "Autoload", Description = "Autoload cleared.", Duration = 2})
        end
    })
end
