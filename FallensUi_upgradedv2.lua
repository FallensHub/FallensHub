...
function Orionlib_ui_ui:BuildMainFrame()
    ...
    local settingsPanelWidth = 220 * scale_factor
    local settingsPanelHeight = 640 * scale_factor
    ...
    -- ── Full Config Panel (Farm Config style) ──────────────────────────────
    ...
    -- Title row: "Farm Config" with icon
    create("ImageLabel", {
        Image = default_icons.settings,
        BackgroundTransparency = 1,
        Position = UDim2.new(0, 10, 0, rowY + 1 * scale_factor),
    ...
    create("TextLabel", {
        ...
        Position = UDim2.new(0, 28 * scale_factor, 0, rowY),
        Size = UDim2.new(1, -38 * scale_factor, 0, 15 * scale_factor),
        TextSize = 13 * scale_factor, TextXAlignment = Enum.TextXAlignment.Left,
        Parent = self.settings_panel
    })
    rowY = rowY + 20 * scale_factor

    -- "Select Config" label + dropdown button
    create("TextLabel", {
        FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold),
        Text = "Select Config",
    ...
    local cfgSelectBtn = create("Frame", {
        BackgroundColor3 = Color3.fromRGB(28, 28, 28),
        AnchorPoint = Vector2.new(1, 0),
        Position = UDim2.new(1, -10, 0, rowY - 1 * scale_factor),
        Size = UDim2.new(0, 72 * scale_factor, 0, 16 * scale_factor),
        Parent = self.settings_panel
    })
    create("UICorner", {CornerRadius = UDim.new(1, 0), Parent = cfgSelectBtn})
    create("UIStroke", {Color = Color3.fromRGB(50, 50, 50), Parent = cfgSelectBtn})
    local cfgSelectLabel = create("TextLabel", {
        FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold),
        Text = "None",
        TextColor3 = Color3.fromRGB(200, 200, 200), BackgroundTransparency = 1,
        Position = UDim2.new(0, 6, 0, 0),
        Size = UDim2.new(1, -18 * scale_factor, 1, 0),
        TextSize = 10 * scale_factor, TextXAlignment = Enum.TextXAlignment.Left,
        TextTruncate = Enum.TextTruncate.AtEnd,
        Parent = cfgSelectBtn
    })
    create("ImageLabel", {
        Image = default_icons.dropdown_arrow,
        ImageColor3 = Color3.fromRGB(140, 140, 140), BackgroundTransparency = 1,
        AnchorPoint = Vector2.new(1, 0.5),
    ...
    create("TextLabel", {
        ...
        TextColor3 = Color3.fromRGB(160, 160, 160), BackgroundTransparency = 1,
        Position = UDim2.new(0, 10, 0, rowY),
        Size = UDim2.new(1, -20, 0, 13 * scale_factor),
        TextSize = 11 * scale_factor, TextXAlignment = Enum.TextXAlignment.Left,
        Parent = self.settings_panel
    })
    rowY = rowY + 15 * scale_factor

    -- Config Name input box
    local cfgNameBox = create("Frame", {
        BackgroundColor3 = Color3.fromRGB(20, 20, 20),
        Position = UDim2.new(0, 10, 0, rowY),
        Size = UDim2.new(1, -20, 0, 22 * scale_factor),
        Parent = self.settings_panel
    })
    create("UICorner", {CornerRadius = UDim.new(0, 6), Parent = cfgNameBox})
    create("UIStroke", {Color = Color3.fromRGB(42, 42, 42), Parent = cfgNameBox})
    local cfgNameInput = create("TextBox", {
        FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold),
        Text = "",
    ...
    rowY = rowY + 26 * scale_factor

    -- Helper: full-width button
    local function makeCfgBtn(label, yPos, bgColor, textColor, clickFn)
        local bg = create("Frame", {
            BackgroundColor3 = bgColor or Color3.fromRGB(26, 26, 26),
            Position = UDim2.new(0, 10, 0, yPos),
            Size = UDim2.new(1, -20, 0, 22 * scale_factor),
            Parent = self.settings_panel
        })
        create("UICorner", {CornerRadius = UDim.new(0, 6), Parent = bg})
        create("UIStroke", {Color = Color3.fromRGB(52, 52, 52), Parent = bg})
        create("TextLabel", {
            FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold),
            Text = label, TextColor3 = textColor or Color3.fromRGB(200, 200, 200),
            BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0),
            TextSize = 11 * scale_factor, Parent = bg
        })
        local btn = create("TextButton", {
            Text = "", BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), Parent = bg
        })
        btn.MouseButton1Click:Connect(function()
            tween_to(bg, {BackgroundColor3 = self.config.AccentColor:Lerp(Color3.fromRGB(20,20,20),0.4)}, 0.08)
            task.delay(0.15, function() tween_to(bg, {BackgroundColor3 = bgColor or Color3.fromRGB(26,26,26)}, 0.12) end)
            if clickFn then clickFn() end
        end)
        btn.MouseEnter:Connect(function() tween_to(bg, {BackgroundColor3 = Color3.fromRGB(38, 38, 38)}, 0.12) end)
        btn.MouseLeave:Connect(function() tween_to(bg, {BackgroundColor3 = bgColor or Color3.fromRGB(26,26,26)}, 0.12) end)
        return bg
    end

    -- Helper: half-width button pair
    local function makeCfgBtnPair(labelL, labelR, yPos, colorL, colorR, tcL, tcR, fnL, fnR)
        local halfW = math.floor((settingsPanelWidth - 24 * scale_factor) / 2)
        local function mkHalf(label, xOff, bgColor, textColor, fn)
            local bg = create("Frame", {
                BackgroundColor3 = bgColor or Color3.fromRGB(26, 26, 26),
                Position = UDim2.new(0, 10 + xOff, 0, yPos),
                Size = UDim2.new(0, halfW, 0, 22 * scale_factor),
                Parent = self.settings_panel
            })
            create("UICorner", {CornerRadius = UDim.new(0, 6), Parent = bg})
            create("UIStroke", {Color = Color3.fromRGB(52, 52, 52), Parent = bg})
            ...
            local btn = create("TextButton", {Text="", BackgroundTransparency=1, Size=UDim2.new(1,0,1,0), Parent=bg})
            btn.MouseButton1Click:Connect(function()
                tween_to(bg, {BackgroundColor3 = self.config.AccentColor:Lerp(Color3.fromRGB(20,20,20),0.4)}, 0.08)
                task.delay(0.15, function() tween_to(bg, {BackgroundColor3 = bgColor or Color3.fromRGB(26,26,26)}, 0.12) end)
                if fn then fn() end
        ...
        mkHalf(labelL, 0, colorL, tcL, fnL)
        mkHalf(labelR, halfW + 4 * scale_factor, colorR, tcR, fnR)
    end

    -- Track selected config name
    local _selectedConfigName = ""
    local function getActiveCfgName()
        local typed = cfgNameInput.Text ~= "" and cfgNameInput.Text or nil
        return typed or (_selectedConfigName ~= "" and _selectedConfigName or self._autoConfigName)
    end

    -- Row 1: Refresh Configs | Load Config
    makeCfgBtnPair(
        "Refresh Configs", "Load Config", rowY,
        Color3.fromRGB(26,26,26), Color3.fromRGB(26,26,26),
        Color3.fromRGB(200,200,200), Color3.fromRGB(200,200,200),
        function()
            -- Refresh: list files
            local folder = get_config_folder()
            local names = {}
            ...
            self:Notify({Title = "Configs", Description = display, Duration = 4})
        end,
        function()
            local ok, msg = self:LoadConfig(getActiveCfgName())
            self:Notify({Title = "Config", Description = ok and "Config loaded!" or ("Load failed: " .. tostring(msg)), Duration = 3})
        end
    )
    rowY = rowY + 26 * scale_factor

    -- Row 2: Save / Create | Delete Config
    makeCfgBtnPair(
        "Save / Create", "Delete Config", rowY,
        Color3.fromRGB(26,26,26), Color3.fromRGB(30, 18, 18),
        Color3.fromRGB(200,200,200), Color3.fromRGB(220, 60, 60),
        function()
            local name = getActiveCfgName()
            if cfgNameInput.Text ~= "" then name = cfgNameInput.Text end
            local ok, msg = self:SaveConfig(name)
            self:Notify({Title = "Config", Description = ok and ("Saved: " .. name) or ("Save failed: " .. tostring(msg)), Duration = 3})
        end,
        function()
            local name = getActiveCfgName()
            ...
            self:Notify({Title = "Config", Description = deleted and ("Deleted: " .. name) or "File not found.", Duration = 3})
        end
    )
    rowY = rowY + 30 * scale_factor
    ...
    -- Import Code text box
    local importCodeBox = create("Frame", {
        BackgroundColor3 = Color3.fromRGB(20, 20, 20),
        Position = UDim2.new(0, 10, 0, rowY),
        Size = UDim2.new(1, -20, 0, 22 * scale_factor),
        Parent = self.settings_panel
    })
    create("UICorner", {CornerRadius = UDim.new(0, 6), Parent = importCodeBox})
    create("UIStroke", {Color = Color3.fromRGB(42, 42, 42), Parent = importCodeBox})
    local importCodeInput = create("TextBox", {
        FontFace = Font.new("rbxasset://fonts/families/GothamSSm.json", Enum.FontWeight.SemiBold),
        Text = "",
        ...
        Size = UDim2.new(1, -14, 1, 0),
        TextXAlignment = Enum.TextXAlignment.Left,
        Parent = importCodeBox
    })
    rowY = rowY + 26 * scale_factor

    -- Row 3: Import Config | Export to Clipboard
    makeCfgBtnPair(
        "Import Config", "Export to Clipboard", rowY,
        Color3.fromRGB(26,26,26), Color3.fromRGB(26,26,26),
        Color3.fromRGB(200,200,200), Color3.fromRGB(200,200,200),
        function()
            local code = importCodeInput.Text
            if code == "" then
                self:Notify({Title = "Import", Description = "Paste a config code first.", Duration = 2})
                return
            end
            local okDecode, data = pcall(function()
            ...
            if not okDecode or type(data) ~= "table" or type(data.controls) ~= "table" then
                self:Notify({Title = "Import", Description = "Invalid config code.", Duration = 3})
                return
            end
            self._isApplyingConfig = true
            for flag, rawValue in pairs(data.controls) do
            ...
            self._isApplyingConfig = false
            self:Notify({Title = "Import", Description = "Config imported!", Duration = 2})
        end,
        function()
            local payload = {version = 1, ui = self.config.Name, controls = {}}
            for flag, control in pairs(self._trackedControls) do
                local ok, value = pcall(control.get)
            ...
            if ok and setclipboard then
                ...
                self:Notify({Title = "Export", Description = "Config copied to clipboard!", Duration = 2})
            else
                self:Notify({Title = "Export", Description = "Clipboard not available.", Duration = 2})
            end
        end
    )
    rowY = rowY + 26 * scale_factor

    -- Row 4: Set Autoload | Clear Autoload
    makeCfgBtnPair(
        "Set Autoload", "Clear Autoload", rowY,
        Color3.fromRGB(26,26,26), Color3.fromRGB(26,26,26),
        Color3.fromRGB(200,200,200), Color3.fromRGB(200,200,200),
        function()
            local name = getActiveCfgName()
            self._autoConfigName = sanitize_config_name(name)
            self._autoConfigEnabled = true
            self._autoConfigLoadAttempted = false
            self:Notify({Title = "Autoload", Description = "Set autoload: " .. name, Duration = 2})
        end,
        function()
            ...
            self._autoConfigEnabled = false
            self._autoConfigLoadAttempted = true
            self:Notify({Title = "Autoload", Description = "Autoload cleared.", Duration = 2})
        end
    )
    rowY = rowY + 26 * scale_factor
